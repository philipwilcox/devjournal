...
directly relevant---it features a section "handling adapters for widget" for listview/gridview. This
gives the following samples, which I'm going to copy here for posterity:
...
In `AppWidgetListView` adapter class:
```java
@Override
public int getCount() {
...
in `AppWidgetProvider` for click handling to open activity with right data:
```java
...
Anyway, that fixes it, and now I crash it cause I haven't implemented the Factory:

```
E/RapidTimer: initializing TimerListWidgetService
E/RapidTimer: Context in TimerLIstWidgetService is android.app.Application@5878e14
E/RapidTimer: initializing TimerListWidgetFactory
E/RapidTimer: onCreate, Context is android.app.Application@5878e14
E/RapidTimer: onDataSetChanged
E/RapidTimer: hasStableIds
E/RapidTimer: getViewTypeCount
E/RapidTimer: getCount
E/RapidTimer: getLoadingView
E/JavaBinder: *** Uncaught remote exception!  (Exceptions are not yet supported across processes.)
              kotlin.NotImplementedError: An operation is not implemented: not implemented
                  at com.mopeyphil.rapidtimer.TimerListWidgetFactory.getLoadingView(TimerListWidgetFactory.kt:23)
```

Oh, and this was easier than I expected to fix: I changed getLoadingView to return a null optional
(data is local, no need for loading view) and it works as expected with some trivial dummy data
tossed in!
